...
(function () {
  ...
  var T = {
    ...
    // ── Login: role picker + placeholders + missing links (fragment-level fixes) ────
    "Tipo de acceso": ["Access type", "Тип доступу"],
    "¿No tienes cuenta? Crea tu empresa →": ["Don't have an account? Create your company →", "Немає акаунта? Створіть компанію →"],
    "tu@empresa.com": ["you@company.com", "ви@компанія.com"],
    "p. ej. mi-empresa-a1b2": ["e.g. my-company-a1b2", "напр. моя-компанія-a1b2"],
    "No se pudo iniciar sesión.": ["Could not sign in.", "Не вдалося увійти."],

    // ── Signup: subtitle, success screen, missing links (fragment-level fixes) ──────
    "Crea tu empresa y empieza gratis": ["Create your company and start for free", "Створіть компанію та почніть безкоштовно"],
    "Empresa creada. Guarda tu ": ["Company created. Save your ", "Компанію створено. Збережіть "],
    "código de empresa": ["company code", "код компанії"],
    " — lo necesitarás para iniciar sesión:": [" — you'll need it to log in later:", " — він знадобиться для входу:"],
    "Ir a la plataforma": ["Go to platform", "Перейти до платформи"],
    "¿Ya tienes cuenta? Inicia sesión": ["Already have an account? Log in", "Вже маєте акаунт? Увійти"],
    "No se pudo crear la empresa.": ["Couldn't create the company.", "Не вдалося створити компанію."],

    // ── Platform shell: aria-label / title attributes (fragment-level fixes) ───────
    "Abrir menú": ["Open menu", "Відкрити меню"],
    "Ir al panel": ["Go to dashboard", "Перейти до панелі"],
    "Cerrar menú": ["Close menu", "Закрити меню"],

    // ── Points/Stats: plain-text section headers missed by the walker split ─────────
    "Ranking de trabajadores": ["Worker ranking", "Рейтинг працівників"],
    "Requiere atención": ["Needs attention", "Потребує уваги"],
    "Elige un archivo (.xlsx, .xls, .csv)": ["Choose a file (.xlsx, .xls, .csv)", "Оберіть файл (.xlsx, .xls, .csv)"],
    "Atrás": ["Back", "Назад"],

    // ── PDF settings placeholders ────────────────────────────────────────────────
    "Albarán de Visita": ["Visit Delivery Note", "Накладна візиту"],
    "Documento generado automáticamente por StarX como justificante de visita.": [
      "Document automatically generated by StarX as proof of visit.",
      "Документ автоматично згенеровано StarX як підтвердження візиту."],

    // ── Bulk delete (Points/Workers) ─────────────────────────────────────────────
    "¿Borrar {n} puntos seleccionados? Esta acción no se puede deshacer.": [
      "Delete {n} selected points? This action cannot be undone.",
      "Видалити {n} вибраних точок? Цю дію не можна скасувати."],
    "{n} puntos borrados": ["{n} points deleted", "{n} точок видалено"],
    "¿Borrar {n} trabajadores seleccionados? Esta acción no se puede deshacer.": [
      "Delete {n} selected workers? This action cannot be undone.",
  ...
  function translateEl(el) {
    ...
    if (el.getAttribute && el.getAttribute("aria-label")) translateAttr(el, "aria-label");
    if (el.getAttribute && el.getAttribute("title")) translateAttr(el, "title");
  }
  function walk(root) {
